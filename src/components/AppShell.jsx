...
function NavList({ activeView, onNavigate, collapsed, onPick, locked }) {
  const Item = ({ item, itemLocked }) => {
    const active = !itemLocked && activeView === item.id;
    const Icon = item.icon;
    return (
      <button
        onClick={() => { if (itemLocked) return; onNavigate(item.id); onPick?.(); }}
        disabled={itemLocked}
        title={collapsed ? item.label : itemLocked ? "Run a crawl to unlock" : undefined}
        className={`group relative flex w-full items-center gap-3 rounded-md px-2.5 py-2 transition-colors ${
          itemLocked
            ? "cursor-not-allowed opacity-35"
            : active
              ? "text-foreground"
              : "text-fg-secondary hover:text-foreground hover:bg-[var(--surface-1)]"
        }`}
      >
        {active && (
          <motion.div
      ...
      <nav className="flex flex-1 flex-col gap-0.5 overflow-y-auto px-2 py-1.5">
        {NAV_GROUPS.map((g) => (
          <div key={g.label} className="mb-1.5">
            {!collapsed && <div className="eyebrow px-2.5 pb-1">{g.label}</div>}
            {g.items.map((n) => <Item key={n.id} item={n} itemLocked={locked} />)}
          </div>
        ))}
        {locked && !collapsed && (
          <div className="mx-0.5 mb-1 flex items-center gap-2 rounded-md border border-[var(--border-subtle)] bg-[var(--surface-1)] px-2.5 py-2 text-[11px] leading-snug text-fg-secondary">
            <Lock className="h-3 w-3 shrink-0" style={{ color: "var(--accent-brand)" }} />
            <span>Run a crawl to unlock modules</span>
          </div>
        )}
      </nav>
      <div className="flex flex-col gap-0.5 border-t border-[var(--border-subtle)] px-2 py-2">
        {!collapsed && <div className="eyebrow px-2.5 pb-1">System</div>}
        {NAV_SYSTEM.map((n) => <Item key={n.id} item={n} itemLocked={false} />)}
      </div>
    </>
  );
}

export default function AppShell({ activeView, onNavigate, onOpenCommand, domain, locked, children }) {
        ...
        <Drawer open={drawerOpen} onOpenChange={setDrawerOpen}>
          <DrawerContent className="bg-[var(--sidebar-background)] text-foreground">
            ...
            <div className="px-2 pb-6">
              <NavList activeView={activeView} onNavigate={onNavigate} collapsed={false} locked={locked} onPick={() => setDrawerOpen(false)} />
            </div>
          </DrawerContent>
        </Drawer>
      </div>
  ...
  return (
    <div className="flex h-screen w-full overflow-hidden bg-background text-foreground">
      ...
      >
        ...
        <NavList activeView={activeView} onNavigate={onNavigate} collapsed={collapsed} locked={locked} />
